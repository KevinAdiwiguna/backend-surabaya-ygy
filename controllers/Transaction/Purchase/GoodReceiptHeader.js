...
export const getGoodReceiptDetail = async (req, res) => {
    try {
        ...
        const mergedPurchaseOrderd = getPurchaseOrderd.map(purchaseOrder => {
            const matchingSalesOrder = getSalesOrder.find(salesOrder => salesOrder.Number === purchaseOrder.Number);
        ...
        const uniqueNumbers = [...new Set(getGoodReceiptDetail.map(item => item.Number))];

        const combinedData = {};

        uniqueNumbers.forEach(number => {
            const itemsWithSameNumber = getGoodReceiptDetail.filter(item => item.Number === number);

            const totalQty = itemsWithSameNumber.reduce((acc, item) => acc + parseFloat(item.Qty), 0);

            const matchingPurchaseOrder = mergedPurchaseOrderd.find(purchaseOrder => purchaseOrder.Number === number);
